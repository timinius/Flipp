const express = require('express');
const Groq = require('groq-sdk');
const { getDB } = require('../db/init');

const router = express.Router();

function formatPrice(p) {
  return new Intl.NumberFormat('ru-RU').format(p) + ' ₽';
}

const SYSTEM_PROMPT = `Ты — умный помощник по подбору товаров на маркетплейсе OCKO.

Твоя задача: понять что нужно пользователю (даже если он говорит косвенно — «хочу научиться петь», «делаю ремонт», «ищу подарок маме») и порекомендовать подходящие товары из предоставленного списка.

Правила:
1. Всегда отвечай на русском языке
2. Будь дружелюбным и конкретным
3. Упоминай товары по названию и цене
4. Если запрос косвенный («хочу похудеть») — объясни какие товары помогут и почему
5. Если товаров нет — предложи попробовать другой запрос
6. На вопросы НЕ о товарах (политика, погода, рецепты и т.д.) отвечай ТОЛЬКО: «Я помогаю с подбором товаров на OCKO. Что ищете?»
7. Отвечай кратко — максимум 3-4 предложения + список товаров`;

function searchProducts(query, db) {
  // Извлекаем ключевые слова для поиска
  const words = query.toLowerCase()
    .replace(/[^\wа-яёa-z\s]/gi, ' ')
    .split(/\s+/)
    .filter(w => w.length > 2);

  if (words.length === 0) {
    return db.prepare(`
      SELECT p.id, p.title, p.price, p.city, p.condition,
             c.name as category_name,
             (SELECT url FROM product_images WHERE product_id = p.id AND is_primary = 1 LIMIT 1) as image
      FROM products p LEFT JOIN categories c ON p.category_id = c.id
      WHERE p.status = 'active' ORDER BY p.views DESC LIMIT 6
    `).all();
  }

  const clauses = words.map(() => '(p.title LIKE ? OR p.description LIKE ? OR c.name LIKE ?)').join(' OR ');
  const params = words.flatMap(w => [`%${w}%`, `%${w}%`, `%${w}%`]);

  const results = db.prepare(`
    SELECT p.id, p.title, p.price, p.city, p.condition, p.description,
           c.name as category_name,
           u.name as seller_name, u.rating as seller_rating,
           (SELECT url FROM product_images WHERE product_id = p.id AND is_primary = 1 LIMIT 1) as image
    FROM products p
    LEFT JOIN categories c ON p.category_id = c.id
    LEFT JOIN users u ON p.seller_id = u.id
    WHERE p.status = 'active' AND (${clauses})
    ORDER BY p.views DESC LIMIT 8
  `).all(...params);

  // Если мало результатов — добавляем популярные
  if (results.length < 3) {
    const existing = new Set(results.map(r => r.id));
    const popular = db.prepare(`
      SELECT p.id, p.title, p.price, p.city, p.condition,
             c.name as category_name,
             (SELECT url FROM product_images WHERE product_id = p.id AND is_primary = 1 LIMIT 1) as image
      FROM products p LEFT JOIN categories c ON p.category_id = c.id
      WHERE p.status = 'active' ORDER BY p.views DESC LIMIT 6
    `).all();
    popular.forEach(p => { if (!existing.has(p.id)) results.push(p); });
  }

  return results.slice(0, 8);
}

router.post('/chat', async (req, res) => {
  const { message, history = [] } = req.body;
  if (!message?.trim()) return res.status(400).json({ error: 'Сообщение пустое' });

  const apiKey = process.env.GROQ_API_KEY;
  if (!apiKey) {
    return res.status(503).json({ error: 'GROQ_API_KEY не настроен в переменных окружения Railway' });
  }

  try {
    const db = getDB();
    const products = searchProducts(message, db);

    const productContext = products.length > 0
      ? '\n\nДОСТУПНЫЕ ТОВАРЫ (используй их для рекомендаций):\n' +
        products.map((p, i) =>
          `${i + 1}. "${p.title}" — ${formatPrice(p.price)} | ${p.condition === 'new' ? 'Новый' : 'Б/у'} | ${p.city}${p.description ? ' | ' + p.description.slice(0, 80) : ''}`
        ).join('\n')
      : '\n\nТовары по запросу не найдены. Сообщи об этом и предложи поискать иначе.';

    const groq = new Groq({ apiKey });

    const messages = [
      ...history.slice(-8).map(m => ({ role: m.role, content: m.content })),
      { role: 'user', content: message },
    ];

    const response = await groq.chat.completions.create({
      model: 'llama-3.3-70b-versatile',
      max_tokens: 500,
      temperature: 0.7,
      messages: [
        { role: 'system', content: SYSTEM_PROMPT + productContext },
        ...messages,
      ],
    });

    const reply = response.choices[0]?.message?.content || 'Не удалось получить ответ';
    res.json({ reply, products: products.slice(0, 4) });

  } catch (e) {
    console.error('Groq error:', e.message);
    res.status(500).json({ error: 'Ошибка AI: ' + e.message });
  }
});

module.exports = router;
